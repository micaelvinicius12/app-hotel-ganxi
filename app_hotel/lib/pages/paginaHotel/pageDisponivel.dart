...
class Disponivel extends StatefulWidget {
  Disponivel({
    ...
    required this.quartosDisponivel,
  });

  Color corDisponivel;
  Color corOcupado;
  Color corausente;
  List<Map<String, dynamic>> quartosDisponivel;
...
class _DisponivelState extends State<Disponivel> {
  ...
  vericaDisponivel() {
    ...
    for (Map<String, dynamic> el in widget.quartosDisponivel) {
      if (el["cor"] == 0) {
        dadosOut.add(el);
      }
    }
...
// ignore: unused_element
  _salvarquivo() async {
    ...
    String dadosjason = json.encode(widget.quartosDisponivel);
